                                     ...
                                     bool aInvertY, bool aBold )
{
    ...
    fmt::format_to( std::back_inserter( buffer ), "{:.3f} w ", lw );
    ...
    for( const std::vector<VECTOR2D>& stroke : *aGlyph )
    {
        ...
        {
            double x = point.x * aUnitsPerEm;
            double y = point.y * aUnitsPerEm;

            if( aInvertY )
            {
                y = -y; // Mirror vertically about baseline (y=0)
                y += ADVANCED_CFG::GetCfg().m_PDFStrokeFontYOffset * aUnitsPerEm;
            }
...
PDF_STROKE_FONT_SUBSET::PDF_STROKE_FONT_SUBSET( const KIFONT::STROKE_FONT* aFont,
                                                double aUnitsPerEm, unsigned aSubsetIndex,
...
void PDF_STROKE_FONT_MANAGER::Reset()
{
    ...
    if( m_font )
    {
        auto newSubset = std::make_unique<PDF_STROKE_FONT_SUBSET>( m_font.get(), m_unitsPerEm, m_nextSubsetIndex++, false, false );
        m_subsets.emplace_back( std::move( newSubset ) );
    }
}
...
{
    // Default style (no bold/italic) uses legacy m_subsets for backward compatibility.
    if( !aBold && !aItalic )
    {
        for( const std::unique_ptr<PDF_STROKE_FONT_SUBSET>& subset : m_subsets )
        {
...
std::vector<PDF_STROKE_FONT_SUBSET*> PDF_STROKE_FONT_MANAGER::AllSubsets() const
{
    ...
    out.reserve( m_subsets.size() );
    for( const auto& up : m_subsets )
