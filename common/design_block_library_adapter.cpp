...
void DESIGN_BLOCK_LIBRARY_ADAPTER::AsyncLoad()
{
    ...
    for( const LIBRARY_TABLE_ROW* row : rows )
    {
        ...
        libNamesCurrentlyValid.insert( libName );

        // TODO(JE) library tables -- check for modified global files
        if( check( libName, m_libraries, m_libraries_mutex ) )
        {
            --m_loadTotal;
            continue;
        }

        if( check( libName, GlobalLibraries, GlobalLibraryMutex ) )
        {
            --m_loadTotal;
            continue;
        }

        m_futures.emplace_back( tp.submit(
            [this]( const wxString& aNickname, LIBRARY_TABLE_SCOPE aScope )
            {
                if( m_abort.load() )
                    return;
                ...
                if( result.has_value() )
                {
                    ...
                    try
                    {
                        ...
                        wxLogTrace( traceLibraries, "DB: %s: library enumerated %zu items", aNickname, dummyList.size() );
                    ...
                    catch( IO_ERROR& e )
                    {
                        ...
                        wxLogTrace( traceLibraries, "DB: %s: plugin threw exception: %s", aNickname, e.What() );
                    }
                }
                else
                {
                    ...
                    {
                        wxLogTrace( traceLibraries, "DB: project library error: %s: %s", aNickname, result.error().message );
