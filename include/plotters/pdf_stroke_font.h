...
public:
    ...
    std::vector<std::unique_ptr<PDF_STROKE_FONT_SUBSET>>& Subsets() { return m_subsets; }
...
private:
    ...
    std::vector<std::unique_ptr<PDF_STROKE_FONT_SUBSET>> m_subsets; // legacy default style group (bold/italic false)
    std::map<unsigned, STYLE_GROUP>                      m_styleGroups; // additional style groups
};
