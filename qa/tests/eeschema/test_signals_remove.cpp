...
BOOST_FIXTURE_TEST_CASE( RemoveFromSignal_DisablesPropagationAndSplitsGroup, SIGNALS_REMOVE_TEST_FIXTURE )
{
    ...
    // Use the base four-nets series chain example
    ...
    // Sanity: ensure we begin with a signal of 4 nets
    size_t fourCount = 0;
    ...
    // Disable propagation on any 2-pin symbol bridging the first net of that signal
    ...
    auto effectiveNetNameForPin = [&]( SCH_PIN* aPin ) -> wxString {
        // Prefer subgraph's resolved name to match RebuildSignals behavior
        ...
        // Fall back to a stable default from the pin
    ...
    // Use the current sheet path to resolve connections consistently
        wxString nameA = effectiveNetNameForPin( pins[0] );
        wxString nameB = effectiveNetNameForPin( pins[1] );

        if( ( nameA == firstNet && nameB != firstNet )
            || ( nameB == firstNet && nameA != firstNet ) )
        {
            sym->SetPassthrough( false );
            wxLogTrace( "KICAD_SCH_HIGHLIGHT", "Test: disabled passthrough on symbol=%p for firstNet='%s' (a='%s', b='%s')",
                        (void*) sym, firstNet, nameA, nameB );
        disabledCount++;
        }
    }
    ...
    // Diagnostics: list symbols and propagate flags after disabling
    for( SCH_ITEM* item : screen->Items().OfType( SCH_SYMBOL_T ) )
    {
        SCH_SYMBOL* sym = static_cast<SCH_SYMBOL*>( item );
    wxLogTrace( "KICAD_SCH_HIGHLIGHT", "Test: symbol=%p mode=%d", (void*) sym,
            (int) sym->GetPassthroughMode() );
    }

    // Recalculate and ensure there's no longer a 4-net signal
    graph->Recalculate( sheets, /*aUnconditional=*/true );
