...
BOOST_AUTO_TEST_CASE( ExtraSignalLengthExcludedSet )
{
    ...
    PNS_KICAD_IFACE_BASE ifaceBase; // base exposes GetNetChainAggregate for testing without full tool setup
    ifaceBase.SetBoard( &board );
    long long extraLen = 0; long long extraDelay = 0;
    bool agg = ifaceBase.GetNetChainAggregate( nP, nN, extraLen, extraDelay );
    BOOST_CHECK( agg );
