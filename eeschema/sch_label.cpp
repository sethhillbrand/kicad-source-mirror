...
SCH_NETCHAIN_LABEL::SCH_NETCHAIN_LABEL( const VECTOR2I& pos, const wxString& text ) :
        SCH_LABEL_BASE( pos, text, SCH_NETCHAIN_LABEL_T )
{
    m_layer      = LAYER_LOCLABEL;
    m_shape      = LABEL_FLAG_SHAPE::L_INPUT;
    m_isDangling = true;
...
void SCH_NETCHAIN_LABEL::Serialize( google::protobuf::Any &aContainer ) const
{
    kiapi::schematic::types::LocalLabel label;
...
bool SCH_NETCHAIN_LABEL::Deserialize( const google::protobuf::Any &aContainer )
{
    kiapi::schematic::types::LocalLabel label;
...
const BOX2I SCH_NETCHAIN_LABEL::GetBodyBoundingBox( const RENDER_SETTINGS* aSettings ) const
{
    BOX2I rect = GetTextBox( aSettings );
...
wxString SCH_NETCHAIN_LABEL::GetItemDescription( UNITS_PROVIDER* aUnitsProvider, bool aFull ) const
{
    return wxString::Format( _( "Net Chain Label '%s'" ),
                             aFull ? GetShownText( false ) : KIUI::EllipsizeMenuText( GetText() ) );
}


BITMAPS SCH_NETCHAIN_LABEL::GetMenuImage() const
{
    return BITMAPS::add_line_label;
}
