...
{
    // Try to get the library table
    SYMBOL_LIB_TABLE* libTable = PROJECT_SCH::SchSymbolLibTable( &Prj() );
    if( !libTable )
    {
        wxMessageBox( _( "Symbol library table not found." ) );
        return;
    }
    ...
    try
    {
        // Attempt to get all symbol names from the library
        wxArrayString symbolNames;
        libTable->EnumerateSymbolLib( m_libId, symbolNames );
        ...
        // Load each symbol and add it to our list
        for( const wxString& symbolName : symbolNames )
        {
            LIB_ID      symbolId( m_libId, symbolName );
            LIB_SYMBOL* symbol = nullptr;

            try
            {
                symbol = libTable->LoadSymbol( symbolId );
