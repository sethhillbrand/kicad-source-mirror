...
// Forward declaration (implemented in pin_layout_cache.cpp)
                                    ...
                                    KIFONT::FONT* aFont, const KIFONT::METRICS& aFontMetrics )
{
    ...
    wxArrayString parts; wxStringSplit( inner, parts, ',' );
    if( parts.empty() )
    ...
    for( size_t i = 0; i < parts.size(); ++i )
    {
        ...
        if( i > 0 )
            result += "\n "; // leading space for visual indent (matches heuristic in tests)
        result += line;
    }
    result += "}";
    return result;
