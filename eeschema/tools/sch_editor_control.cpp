    ...
    else
    {
        ...
        if( connName != editFrame->GetHighlightedConnection() )
        {
            wxLogTrace( "KICAD_SCH_HIGHLIGHT", "highlightNet: setting highlighted connection to %s",
                        connName );
            editorControl->SetHighlightBusMembers( false );
            // Clear any previous net chain highlight when switching to net highlight
            editFrame->SetHighlightedNetChain( wxEmptyString );
        ...
        {
            // Same net requested again. Try to expand to the containing net chain if available.
            wxLogTrace( "KICAD_SCH_HIGHLIGHT", "highlightNet: same net re-invoked; trying to expand to net chain" );
            CONNECTION_GRAPH* graph = editFrame ? editFrame->Schematic().ConnectionGraph() : nullptr;

            if( graph )
            {
                // Build net chains on-demand if needed
                if( graph->GetNetChains().empty() )
                {
                    wxLogTrace( "KICAD_SCH_HIGHLIGHT", "highlightNet: net chains empty; rebuilding before expand" );
                    SCH_SHEET_LIST sheets = editFrame->Schematic().Hierarchy();
                    graph->Recalculate( sheets, /*aUnconditional=*/true );
                }

                if( SCH_NETCHAIN* netchain = graph->GetNetChainForNet( connName ) )
                {
                    // Only switch if this net is indeed part of a multi-net chain or any chain
                    wxString netchainName = netchain->GetName();
                    wxLogTrace( "KICAD_SCH_HIGHLIGHT", "highlightNet: expanding to net chain '%s' (nets=%zu)",
                                netchainName, netchain->GetNets().size() );
                    editFrame->SetHighlightedConnection( wxEmptyString );
                ...
                else
                {
                    ...
                    wxLogTrace( "KICAD_SCH_HIGHLIGHT", "highlightNet: no net chain found; toggling bus members" );
...
int SCH_EDITOR_CONTROL::HighlightNetChain( const TOOL_EVENT& aEvent )
{
    ...
    {
        wxLogTrace( "KICAD_SCH_HIGHLIGHT", "HighlightNetChain: net chains empty; calling Recalculate(unconditional=true)" );
        SCH_SHEET_LIST sheets = editFrame->Schematic().Hierarchy();
        graph->Recalculate( sheets, /*aUnconditional=*/true );
    }
    ...
    if( item )
    {
        ...
        if( conn )
        {
            ...
            if( !sig && graph )
            {
                wxLogTrace( "KICAD_SCH_HIGHLIGHT", "HighlightNetChain: no net chain found; Recalculate(unconditional=true) and retry" );
                SCH_SHEET_LIST sheets = editFrame->Schematic().Hierarchy();
                graph->Recalculate( sheets, /*aUnconditional=*/true );
            ...
            if( sig )
            {
                ...
                wxLogTrace( "KICAD_SCH_HIGHLIGHT", "HighlightNetChain: found net chain=%s", netChainName );
            }
            else
            {
                wxLogTrace( "KICAD_SCH_HIGHLIGHT", "HighlightNetChain: no net chain for net=%s; falling back to net highlight", conn->Name() );
                editFrame->SetHighlightedNetChain( wxEmptyString );
...
int SCH_EDITOR_CONTROL::RemoveFromNetChain( const TOOL_EVENT& aEvent )
{
    ...
    for( SCH_ITEM* item : screen->Items().OfType( SCH_SYMBOL_T ) )
    {
        ...
        // this symbol is bridging the selected net into its net chain.
    ...
    if( disabled > 0 )
    {
        // Rebuild connectivity/net chains so the change takes effect
        CONNECTION_GRAPH* graph = schematic.ConnectionGraph();
        if( graph )
        {
            wxLogTrace( "KICAD_SCH_HIGHLIGHT", "RemoveFromNetChain: disabled=%d, rebuilding net chains", disabled );
...
int SCH_EDITOR_CONTROL::ClearHighlight( const TOOL_EVENT& aEvent )
{
    ...
    // Also clear any highlighted net chain explicitly
    if( m_frame )
...
int SCH_EDITOR_CONTROL::UpdateNetHighlighting( const TOOL_EVENT& aEvent )
{
    ...
    wxLogTrace( "KICAD_SCH_HIGHLIGHT", "UpdateNetHighlighting: highlightedConn='%s' highlightedNetChain='%s'",
    ...
    if( !m_frame->GetHighlightedNetChain().IsEmpty() )
    {
        if( SCH_NETCHAIN* sig = connectionGraph->GetNetChainByName( m_frame->GetHighlightedNetChain() ) )
        {
            for( const wxString& n : sig->GetNets() )
                connNames.emplace( n );
            wxLogTrace( "KICAD_SCH_HIGHLIGHT", "UpdateNetHighlighting: added %zu nets from net chain '%s'",
...
int SCH_EDITOR_CONTROL::NameNetChain( const TOOL_EVENT& aEvent )
{
    ...
    if( SCH_NETCHAIN* netChain = graph->GetNetChainForNet( pin->Connection()->Name() ) )
    {
        wxString newName = wxGetTextFromUser( _( "Net chain name:" ), _( "Name Net Chain" ), netChain->GetName() );

        if( !newName.IsEmpty() && newName != netChain->GetName() )
        {
            wxString oldName = netChain->GetName();
            netChain->SetName( newName );

            SCH_SCREEN* screen = editFrame->GetCurrentSheet().LastScreen();

            for( EDA_ITEM* scrItem : screen->Items() )
            {
                SCH_NETCHAIN_LABEL* label = dynamic_cast<SCH_NETCHAIN_LABEL*>( scrItem );
...
int SCH_EDITOR_CONTROL::CreateNetChainBetweenPins( const TOOL_EVENT& aEvent )
{
    ...
    wxString prevHighlightedSig = editFrame->GetHighlightedNetChain();
    ...
    if( name.IsEmpty() )
    {
        ...
        editFrame->SetHighlightedNetChain( prevHighlightedSig );
    ...
    {
        // Replace temporary highlight with new net chain name
