...
// Forward declaration of helper used inside NETCHAINS_MENU::update
class NETCHAINS_MENU;
static bool addCreateNetChainBetweenPinsIfApplicable( NETCHAINS_MENU* aMenu, SCH_EDIT_FRAME* aFrame, const SCH_SELECTION& aSel );

class NETCHAINS_MENU : public ACTION_MENU
{
public:
    NETCHAINS_MENU() : ACTION_MENU( true )
    {
...
protected:
    ACTION_MENU* create() const override { return new NETCHAINS_MENU(); }

    void update() override
    {
        Clear();

        wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] update() invoked" );
        ...
        if( !toolMgr )
        {
            ...
            SetDirty();
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] defer: toolMgr not yet available" );
            return;
        }
        ...
        if( !selTool || !frame )
        {
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] abort: selTool=%p frame=%p", (void*) selTool, (void*) frame );
            return;
        }
        ...
        if( sel.Empty() )
        {
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] abort: empty selection" );
            return; // nothing to show
        }

        wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] selection size=%u", sel.GetSize() );

        CONNECTION_GRAPH* graph = frame->Schematic().ConnectionGraph();
        if( !graph )
        {
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] abort: no connection graph" );
            return;
        }
        ...
        bool inNetChain  = false; // at least one selected item participates in a committed net chain
        bool canName   = false; // we can rename a net chain (single pin with committed net chain)
        bool canRemove = false; // we can remove an item from its net chain
        ...
        for( size_t i = 0; i < sel.GetSize(); ++i )
        {
            ...
            if( !p || !p->Connection() )
            {
                wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] sel[%zu]: not a pin or no connection", i );
                continue;
            }

            wxString netName = p->Connection()->Name();
            bool hasNetChain = graph->GetNetChainForNet( netName );
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] sel[%zu]: pin uuid=%s net=%s committedNetChain=%d", i,
                        p->m_Uuid.AsString(), netName, hasNetChain );
            if( graph->GetNetChainForNet( p->Connection()->Name() ) )
            {
                inNetChain = true;
                canRemove = true; // current remove handler works on a pin in a net chain
        ...
        wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] flags: singlePin=%d inNetChain=%d canName=%d canRemove=%d", singlePin, inNetChain, canName, canRemove );

        // highlightNetChain action: only if we have a committed net chain context
        if( inNetChain )
        {
            Add( SCH_ACTIONS::highlightNetChain );
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] added highlightNetChain" );
        }
        ...
        if( canRemove )
        {
            ...
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] added removeFromNetChain" );
        }

        // Replace terminal pin submenu only when a single pin belonging to a net chain is selected
        if( singlePin && inNetChain )
        {
            Add( m_replaceMenu );
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] added replaceTerminalPin submenu" );
        }
        ...
        if( canName )
        {
            ...
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] added nameNetChain" );
        }

        // createNetChainBetweenPins: two pins selected, share potential (uncommitted) net chain
        if( sel.GetSize() == 2 )
        {
            bool added = addCreateNetChainBetweenPinsIfApplicable( this, frame, sel );
            wxLogTrace( "KICAD_NETCHAINS_MENU", "[NetChainsMenu] createNetChainBetweenPins attempted added=%d", added );
        }
...
bool SCH_SELECTION_TOOL::Init()
{
    ...
    std::shared_ptr<NETCHAINS_MENU> netchainsMenu = std::make_shared<NETCHAINS_MENU>();
    ...
    menu.AddMenu( netchainsMenu.get(),                 ( pinSelection || wireOrBusInSignal ) && SCH_CONDITIONS::Idle, 400 );
