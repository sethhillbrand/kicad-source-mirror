...
bool PNS_KICAD_IFACE_BASE::GetNetChainAggregate( PNS::NET_HANDLE aNetP, PNS::NET_HANDLE aNetN,
                                               long long& aExtraLength, long long& aExtraDelay ) const
{
    ...
    wxString netChain = netP->GetNetChain();

    if( netChain.IsEmpty() || netChain != netN->GetNetChain() )
        return false;
    ...
    for( NETINFO_ITEM* net : m_board->GetNetInfo() )
    {
                ...
                for( BOARD_ITEM* bi : m_board->Tracks() )
                {
                    if( auto tr = dynamic_cast<PCB_TRACK*>( bi ) )
                    {
                        if( tr->GetNetCode() == net->GetNetCode() )
                        {
                            ...
                            {
                                minX = std::min( minX, std::min( sx, ex ) );
                                maxX = std::max( maxX, std::max( sx, ex ) );
                            }
                        }
                    }
                }
        ...
        {
            int count = 0;
            double trk = 0, pad = 0, tDelay = 0, padDelay = 0;
...
bool PNS_KICAD_IFACE::GetNetChainAggregate( PNS::NET_HANDLE aNetP, PNS::NET_HANDLE aNetN,
                                          long long& aExtraLength, long long& aExtraDelay ) const
{
    return PNS_KICAD_IFACE_BASE::GetNetChainAggregate( aNetP, aNetN, aExtraLength, aExtraDelay );
}
