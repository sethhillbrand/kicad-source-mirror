 ...
 */
 ...
 public:
    ...
    // Compute aggregate extra length/delay for other nets sharing the same net chain as the two
    // provided nets (if both belong to the same non-empty net chain). Returns true if aggregation
    // was performed. extraLength/extraDelay exclude the two nets themselves.
    virtual bool GetNetChainAggregate( NET_HANDLE aNetP, NET_HANDLE aNetN,
