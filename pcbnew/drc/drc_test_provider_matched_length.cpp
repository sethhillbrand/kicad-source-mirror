...
bool DRC_TEST_PROVIDER_MATCHED_LENGTH::runInternal( bool aDelayReportMode )
{
    ...
    forEachGeometryItem( { PCB_TRACE_T, PCB_ARC_T, PCB_VIA_T, PCB_PAD_T }, boardCopperLayers,
            [&]( BOARD_ITEM *item ) -> bool
            {
                ...
                for( DRC_CONSTRAINT_T jj : { LENGTH_CONSTRAINT, NETCHAIN_LENGTH_CONSTRAINT, SKEW_CONSTRAINT, VIA_COUNT_CONSTRAINT } )
                {
    ...
    if( !aDelayReportMode )
    {
        ...
        for( std::pair< DRC_RULE* const, std::vector<CONNECTION> > it : matches )
        {
            ...
            std::optional<DRC_CONSTRAINT> signalLengthConstraint = rule->FindConstraint( NETCHAIN_LENGTH_CONSTRAINT );
