...
// Parse the (netchains ...) aggregation block providing net chain names, member nets and optional terminal pads.
void PCB_IO_KICAD_SEXPR_PARSER::parseNETCHAINS_SECTION()
{
    // Tokens inside section: (netchain (name <str>) (members (member (net <code>))...) (terminal_pad <uuid>) (terminal_pad <uuid>))
    for( T token = NextTok(); token != T_EOF; token = NextTok() )
    {
        if( token == T_RIGHT )
            break; // end of (netchains ...)
        ...
        for( int code : netCodes )
        {
            ...
            for( int i = 0; i < padCount && i < 2; ++i )
            {
                ...
                // Try to resolve immediately (the writer places the (netchain ...) section
                ...
                if( PAD* pad = m_board->FindPadByUuid( padUuids[i] ) )
                {
                    ...
                    wxLogTrace( wxT( "KICAD_NETCHAIN_PARSER" ),
                                wxT( "Resolved terminal_pad[%d] %s for net %d (netchain %s)" ), i, padUuids[i].AsString(),
                                code, name );
                }
                else
                {
                    wxLogTrace( wxT( "KICAD_NETCHAIN_PARSER" ),
                                wxT( "Failed immediate resolve terminal_pad[%d] %s for net %d (netchain %s)" ), i,
                                padUuids[i].AsString(), code, name );
                }
            }
            if( padCount == 0 )
            {
                wxLogTrace( wxT( "KICAD_NETCHAIN_PARSER" ), wxT( "NetChain %s had no terminal_pad entries (net %d)" ), name,
                            code );
            }
        }
        wxLogTrace( wxT( "KICAD_NETCHAIN_PARSER" ),
                    wxT( "Parsed NetChain %s with %zu member nets and %d terminal pads" ),
