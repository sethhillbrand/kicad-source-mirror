...
void GENDRILL_WRITER_BASE::buildHolesList( const DRILL_SPAN& aSpan, bool aGenerateNPTH_list )
{
    ...
    if( !aGenerateNPTH_list )
    {
        for( PCB_TRACK* track : m_pcb->Tracks() )
        {
            ...
            if( aSpan.m_IsBackdrill )
            {
                ...
                new_hole.m_PostMachining = secondary.post_machining.value_or( false );
            ...
            new_hole.m_PostMachining = via->Padstack().Drill().post_machining.value_or( false );
            new_hole.m_DrillStart = top_layer;
    ...
    for( unsigned ii = 0; ii < m_holeListBuffer.size(); ii++ )
    {
        ...
        if( m_holeListBuffer[ii].m_PostMachining )
