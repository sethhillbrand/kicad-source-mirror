...
public:
     ...
     *
     * @param NewText UTF-8 string content for the barcode.
     */
     ...
     * to fit the given rectangle and update width/height accordingly.
     *
    ...
    void SetBarcodeText( const wxString& aText ) { SetText( aText ); ComputeBarcode(); }
    void SetShowText( bool aShow ) { m_text.SetVisible( aShow ); ComputeBarcode(); }
    bool GetShowText() const { return m_text.IsVisible(); }

    void SetBarcodeWidth( int aWidth ) { m_width = aWidth; ComputeBarcode(); }
    void SetBarcodeHeight( int aHeight ) { m_height = aHeight; ComputeBarcode(); }

    double GetOrientation() const { return m_text.GetTextAngle().AsDegrees(); }
    void   SetOrientation( double aDegrees )
    {
        EDA_ANGLE newAngle( aDegrees, DEGREES_T );
        EDA_ANGLE oldAngle = m_text.GetTextAngle();
...
private:
    ...
    VECTOR2I       m_total_size; ///< Size of the barcode + margins
